...
class StaffAdmin(BaseUserAdmin):
	...
	exclude = ['last_name','first_name','user_permissions','date_joined']
	...
	fieldsets = (
			("基本資料", {
				...
				'fields': ('username','password','name','gender','birthday','idno','role',
					'mobile','email','fb_url','account','groups','is_active','is_staff',
					'is_superuser')
				}
				),
			)
	...
	add_fieldsets = (
			("基本資料", {
				...
				'fields': ('username','password1','password2','name','gender','birthday','idno','role',
					'mobile','email','fb_url','account','groups','is_active','is_staff','is_superuser')
				}
				),
			)
